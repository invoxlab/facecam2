...
const ProjectList = () => {
  ...
  const { projects, isLoading, createProject, deleteProject } = useProjects('local');
  ...
  const handleCreate = async (name: string) => {
    const project = await createProject(name, 'local');
    setShowForm(false);
        ...
        <div className="max-w-lg mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-invox-blue text-white px-3.5 py-2 rounded-xl text-sm font-medium"
            >
              <Plus size={16} />
              Nouveau
            </button>
        ...
        <div className="max-w-lg mx-auto px-4">
          <div className="flex border-b border-gray-100">
            ...
            >
              A tourner
              {toRecord.length > 0 && (
                <span className="ml-1.5 bg-invox-blue/10 text-invox-blue text-xs rounded-full px-1.5 py-0.5">
                  {toRecord.length}
                </span>
              )}
            </button>
      ...
      <main className="max-w-lg mx-auto px-4 py-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            {tab === 'to-record' ? (
              <>
                <div className="w-16 h-16 bg-invox-blue/10 rounded-2xl flex items-center justify-center mb-4">
                  <Video size={28} className="text-invox-blue" />
                </div>
                ...
                <button
                  onClick={() => setShowForm(true)}
                  className="flex items-center gap-2 bg-invox-blue text-white px-5 py-3 rounded-xl font-medium text-sm"
                >
                  <Plus size={18} />
                  Créer un projet
                </button>
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm p-5 shadow-xl">
            ...
            <form onSubmit={e => { e.preventDefault(); const v = (e.currentTarget.elements.namedItem('name') as HTMLInputElement).value; if (v.trim()) handleCreate(v.trim()); }}>
              <input
                ...
                autoFocus
                className="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-invox-blue text-base mb-4"
              />
              <div className="flex gap-3">
                ...
                <button type="submit" className="flex-1 py-3 rounded-xl bg-invox-blue text-white font-medium text-sm">Créer</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-5 shadow-xl">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmDelete} className="flex-1 py-3 rounded-xl bg-invox-orange text-white font-medium text-sm">Supprimer</button>
            </div>
          </div>
        </div>
      )}
